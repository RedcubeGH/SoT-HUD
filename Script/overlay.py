...
from pixel_utils import get_dyn_x, get_dyn_y, get_dyn_pos_right, get_multiple_pixels, get_sizes, hwnd as pixel_hwnd
...
class Overlay(QtWidgets.QWidget):
    ...
    def set_click_through_native(self):
        try:
            hwnd_win = int(self.winId())
            exStyle = win32gui.GetWindowLong(hwnd_win, win32con.GWL_EXSTYLE)
            ...
            win32gui.SetWindowLong(hwnd_win, win32con.GWL_EXSTYLE, exStyle)
    ...
    def update_loop(self):
        ...
        if Config.calibrated_ammo_colour == (0,0,0):
            ...
            try:
                ...
                print(f"Couldn't calibrate ammo colour: {e}", end="\r", flush=True)
            return

        try:
            foreground = win32gui.GetForegroundWindow()
            if get_multiple_pixels and foreground and get_multiple_pixels is not None:
                # Only run when game is focused and hwnd set
                if get_multiple_pixels and hasattr(__import__('pixel_utils'), 'hwnd'):
                    pass

            # check window & focus
            if hasattr(__import__('pixel_utils'), 'hwnd') and __import__('pixel_utils').hwnd and __import__('win32gui').GetForegroundWindow() == __import__('pixel_utils').hwnd and not Config.show_UI:
                ...
                if len(all_pixels) >= 10:
                    pixel_colour = all_pixels[6] if all_pixels[6] is not None else np.array([0,0,0])
                    control_colour = all_pixels[7] if all_pixels[7] is not None else np.array([0,0,0])
                    additional_check = all_pixels[8] if all_pixels[8] is not None else np.array([0,0,0])
                    regen_control_colour = all_pixels[9] if all_pixels[9] is not None else np.array([0,0,0])
            ...
            print(f"{e} (Game is probably starting right now)", end="\r", flush=True)
