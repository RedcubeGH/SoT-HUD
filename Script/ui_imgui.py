...
def imgui_thread(overlay):
    ...
    if not window:
        print("Failed to create imgui window")
        glfw.terminate()
        return
    ...
    def set_clickthrough(hwnd_local, enabled: bool):
        if enabled:
            exStyle_local = win32gui.GetWindowLong(hwnd_local, win32con.GWL_EXSTYLE)
            exStyle_local |= win32con.WS_EX_LAYERED | win32con.WS_EX_TRANSPARENT
            win32gui.SetWindowLong(hwnd_local, win32con.GWL_EXSTYLE, exStyle_local)
        else:
            exStyle_local = win32gui.GetWindowLong(hwnd_local, win32con.GWL_EXSTYLE)
            exStyle_local &= ~win32con.WS_EX_TRANSPARENT
            win32gui.SetWindowLong(hwnd_local, win32con.GWL_EXSTYLE, exStyle_local)
        ...
        if Config.show_UI:
                ...
                if imgui.begin_menu('File'):
                    ...
                    with imgui.begin_menu('Open Config', True) as open_recent_menu:
                        if open_recent_menu.opened:
                            cfg_dir = os.path.join(script_dir, "..", "YourConfigs")
                            if os.path.exists(cfg_dir):
                                for file in os.listdir(cfg_dir):
                                    changed, _ = imgui.menu_item(file, None, False, True)
                                    if changed:
                                        Config.load_config(os.path.join(cfg_dir, file))
                    imgui.end_menu()
                imgui.end_menu_bar()
            ...
            if imgui.begin_tab_item("Healthbar")[0]:
                ...
                if Config.healthbartoggle:
                    health_rgb = _hex_to_rgb_f(Config.healthcolour)
                    changed, health_rgb = imgui.color_edit3("Health colour", *health_rgb)
                    if changed:
                        Config.healthcolour = _rgb_f_to_hex(health_rgb)
                    ...
                    lowhealth_rgb = _hex_to_rgb_f(Config.lowhealthcolour)
                    changed, lowhealth_rgb = imgui.color_edit3("Low health colour", *lowhealth_rgb)
                    if changed:
                        Config.lowhealthcolour = _rgb_f_to_hex(lowhealth_rgb)
                ...
                if Config.numberhealthtoggle:
                    ...
                    numberhealth_rgb = _hex_to_rgb_f(Config.numberhealthcolour)
                    changed, numberhealth_rgb = imgui.color_edit3("", *numberhealth_rgb)
                    if changed:
                        Config.numberhealthcolour = _rgb_f_to_hex(numberhealth_rgb)
                    for row_idx, row in enumerate(anchor_grid):
                        for col_idx, anchor in enumerate(row):
                            ...
                            if imgui.button(anchor, width=25, height=25):
                                Config.healthanchor = anchor
                            imgui.pop_style_color()
                            if col_idx < len(row) - 1:
                                imgui.same_line()
                    imgui.columns(3, "health_columns", False)
            ...
            if imgui.begin_tab_item("Overheal")[0]:
                ...
                if Config.regentoggle:
                    overheal_rgb = _hex_to_rgb_f(Config.overhealcolour)
                    changed, overheal_rgb = imgui.color_edit3("Overheal colour", *overheal_rgb)
                    if changed:
                        Config.overhealcolour = _rgb_f_to_hex(overheal_rgb)
                    regenbg_rgb = _hex_to_rgb_f(Config.regenbgcolour)
                    changed, regenbg_rgb = imgui.color_edit3("Background colour", *regenbg_rgb)
                    if changed:
                        Config.regenbgcolour = _rgb_f_to_hex(regenbg_rgb)
                ...
                if Config.numberregentoggle:
                    ...
                    numberregen_rgb = _hex_to_rgb_f(Config.numberregencolour)
                    changed, numberregen_rgb = imgui.color_edit3("", *numberregen_rgb)
                    if changed:
                        Config.numberregencolour = _rgb_f_to_hex(numberregen_rgb)
                    for row_idx, row in enumerate(anchor_grid):
                        for col_idx, anchor in enumerate(row):
                            if anchor == Config.regenanchor:
                                ...
                                imgui.push_style_color(imgui.COLOR_BUTTON, 0.4, 0.4, 0.4, 1.0)
                            if imgui.button(anchor, width=25, height=25):
                                Config.regenanchor = anchor
                            imgui.pop_style_color()
                            if col_idx < len(row) - 1:
                                imgui.same_line()
                    imgui.columns(3, "regen_columns", False)
                    imgui.set_column_width(0, 90)
                    imgui.push_item_width(80)
            ...
            if imgui.begin_tab_item("Ammo")[0]:
                ...
                if Config.numberammotoggle:
                    ...
                    numberammo_rgb = _hex_to_rgb_f(Config.numberammocolour)
                    changed, numberammo_rgb = imgui.color_edit3("", *numberammo_rgb)
                    if changed:
                        Config.numberammocolour = _rgb_f_to_hex(numberammo_rgb)
                    for row_idx, row in enumerate(anchor_grid):
                        for col_idx, anchor in enumerate(row):
                            if anchor == Config.ammoanchor:
                                ...
                                imgui.push_style_color(imgui.COLOR_BUTTON, 0.4, 0.4, 0.4, 1.0)
                            if imgui.button(anchor, width=25, height=25):
                                Config.ammoanchor = anchor
                            imgui.pop_style_color()
                            if col_idx < len(row) - 1:
                                imgui.same_line()
                    imgui.columns(3, "ammo_columns", False)
                    imgui.set_column_width(0, 90)
                    imgui.push_item_width(80)
            ...
            if imgui.begin_tab_item("Misc")[0]:
                ...
                if Config.crosshairtoggle:
                    ...
                    crosshair_rgb = _hex_to_rgb_f(Config.crosshaircolour)
                    changed, crosshair_rgb = imgui.color_edit3("Crosshair colour", *crosshair_rgb)
                    if changed:
                        Config.crosshaircolour = _rgb_f_to_hex(crosshair_rgb)
                    crosshairoutline_rgb = _hex_to_rgb_f(Config.crosshairoutlinecolour)
                    changed, crosshairoutline_rgb = imgui.color_edit3("Crosshair outline colour", *crosshairoutline_rgb)
                    if changed:
                        Config.crosshairoutlinecolour = _rgb_f_to_hex(crosshairoutline_rgb)
                ...
                try:
                    ...
                    Config.current_font = overlay.fonts.index(Config.font)
                clicked, Config.current_font = imgui.combo("Font", Config.current_font, overlay.fonts, 30)
